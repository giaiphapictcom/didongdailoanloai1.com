            ...
            <tr>
                ...
                <td><select name="listCat" id="jumpMenu" onchange="MM_jumpMenu('parent', this, 0)">
                        ...
                        <?php foreach ($list_cat as $k => $v) { ?>
                            <option value="<?php echo DOMAINAD; ?>catproducts/index/<?php echo $k; ?>" <?php
                            if ($catId == $k) {
...
<!--                        <th style="text-align:center;"><a href="#" onclick="changepos()">Home</a></th>-->
                        <th>Cập nhật</th>
                        <th>Xử lý</th>
                        <th>Hiển thị</th>
                        <th width="3%">ID</th>
                ...
                <tbody>
                    ...
                    foreach ($Catproduct as $key => $value) {
                        ...
                        <tr>
                            ...
                            <td><a href="<?php echo DOMAINAD ?>catproducts/edit/<?php echo $value['Catproduct']['id']; ?>/<?php echo $value['Catproduct']['parent_id']; ?>" title="Edit"> <?php echo $value['Catproduct']['name']; ?> </a></td>
...
<!--                            <td>
                                <label><input type="radio" name="sphot[<?php echo $value['Catproduct']['id']; ?>]" value="1" id="sphot_0" <?php
                                    if ($value['Catproduct']['hot'] == 1) {
                                        echo 'checked';
                                    }
                                    ?>> Có</label>
                                <label><input name="sphot[<?php echo $value['Catproduct']['id']; ?>]" type="radio" id="sphot_1" value="0" <?php
                                    if ($value['Catproduct']['hot'] == 0) {
                                        echo 'checked';
                                    }
                                    ?>>  Không</label>
                            </td>-->
                            ...
                            <td><!-- Icons -->
                                <a href="<?php echo DOMAINAD ?>catproducts/edit/<?php echo $value['Catproduct']['id'] ?>/<?php echo $value['Catproduct']['parent_id']; ?>" title="Sửa mục này"><img src="<?php echo DOMAINAD ?>images/icons/pencil.png" alt="Sửa" /></a> <a href="javascript:confirmDelete('<?php echo DOMAINAD ?>catproducts/delete/<?php echo $value['Catproduct']['id'] ?>/<?php echo $value['Catproduct']['parent_id']; ?>')" title="Xóa mục này"><img src="<?php echo DOMAINAD ?>images/icons/cross.png" alt="Xóa" /></a></td>
                            <td><?php if ($value['Catproduct']['status'] == 1) { ?>
